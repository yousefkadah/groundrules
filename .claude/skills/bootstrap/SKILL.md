---
name: bootstrap
description: Use ONCE right after running `agentstd init` (or when onboarding an agent to an unfamiliar repo). Scans the codebase and fills in .ai/context.md, replaces the «placeholders», records real cross-cutting rules, and drafts project-specific skills — turning the template into a project-aware guide.
---

# Bootstrap this project's AI guidance

`agentstd init` scaffolds `.ai/` from stack packs, but `context.md` and some rules ship as
...
the human reviews your draft. Then remind them to run `agentstd generate` to re-sync the adapters.

## 1. Scan (read-only)
- [ ] Confirm the stack from manifests and `.ai/.agentstd.json`. Note framework(s), test runner, build tooling.
...
- [ ] Tell the human to run `agentstd generate` so AGENTS.md / CLAUDE.md / Cursor / Copilot / Gemini update.
