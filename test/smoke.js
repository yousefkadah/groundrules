...
// 21. AI-policy guard REFUSES before writing (a warning after mutation is not a guard)
{
  const { execFileSync } = require('child_process');
  const cli = path.join(__dirname, '..', 'bin', 'groundrules.js');
  const dir = scaffold({ 'go.mod': 'module x\n', 'CONTRIBUTING.md': '- **No AI-Generated Code**: Do not submit code generated by AI tools.\n' });
  let code = 0;
  try { execFileSync(process.execPath, [cli, 'init', `--cwd=${dir}`], { stdio: 'pipe' }); } catch (e) { code = e.status; }
  ok(code === 1, 'init refuses (exit 1) when the repo forbids AI contributions');
  ok(!fs.existsSync(path.join(dir, 'AGENTS.md')), 'refusal writes no AGENTS.md');
  ok(!fs.existsSync(path.join(dir, '.ai')), 'refusal writes no .ai/');
  execFileSync(process.execPath, [cli, 'init', '--ignore-ai-policy', `--cwd=${dir}`], { stdio: 'pipe' });
  ok(fs.existsSync(path.join(dir, 'AGENTS.md')), '--ignore-ai-policy proceeds (the human owns the call)');
}
